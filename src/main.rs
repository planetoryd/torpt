...
enum Command {
    /// Enable client mode.
    /// It will listen on a local socket. 
    /// You can connect to it, AS IF it is what the remote PT server is proxying.
    Client {
        ...
        /// successfully
        /// Make sure to wrap it with double quotes
        #[arg(required = true)]
    ...
    Server {
        ...
        /// TCP destination to forward all streams to. 
        /// If not specified, a local socks5 server will be set up, and all streams will be forwarded to it.
        forward: Option<String>,
    },
}

/// Tor pluggable transport driver.
#[derive(Parser)]
...
async fn main() -> Result<()> {
    ...
    match args.command {
        Command::Client {
            ...
            let entry_addr = format!("127.0.0.1:{}", local_port);
            ...
            let client_endpoint = client_pt
                ...
                .endpoint()
                .to_string();
            println!("PT client listening at {}", &client_endpoint);
            println!("Auth {}", obfs4_server_conf);
        ...
        Command::Server {
            ...
            if forward.is_none() {
                let final_socks5_endpoint = format!("127.0.0.1:{}", 0);
                let _ = run_socks5_server(&final_socks5_endpoint).await?;
                forward = Some(final_socks5_endpoint);
            }
            println!("PT server listening on: {}", listen_address);
            launch_obfs4_server_process(obfs4_path, listen_address, forward.unwrap()).await?;
